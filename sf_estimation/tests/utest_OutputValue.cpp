...
namespace OutputValueTest 
{
  ...
  TEST(OutputValueTest, initialization)
  {
    ...
    for (int i = 0; i < 1000; i++)
    {
      ...
      EXPECT_DOUBLE_EQ(empty.getVariance(), -1);
      EXPECT_NEAR(empty.getJitter(), 0, 0.01);
    }
    ...
    EXPECT_NEAR(val.getJitter(), 3, 0.01);
    ...
    EXPECT_NEAR(val.getJitter(), 3+elapsed_time_ms, 0.01);
    ...
    for (int i = 0; i < 1000; i++)
    {
      ...
      EXPECT_NEAR(fill.getJitter(), 0, 0.01);

      fill.setJitter(20);
      
      EXPECT_NEAR(fill.getJitter(), 20, 0.01);
    }
  }
  ...
  TEST(OutputValueTest, operationsOn)
  {
    ...
    EXPECT_NEAR(temp.getJitter(), x.getJitter(), 0.01);
